...
def run_test_integers():
    ...
    actual = integers([[1,2],'Helloo',('hi', 5, 5),[6,7,8],[]])
    print('Expected is:', expected)
...
def run_test_big_letters():
    ...
    #Test 2:
    expected = 'DOG'
    answer = big_letters([(1,2,3),'cool',(1,2),'Dig','nO',(34,56),'Good!'])
    print('Expected is:', expected)
    print('Acutal is:', answer)
    ...
    for k in range(len(sequence_of_sequences)):
        ...
        for j in range (len(sublist)):
            if type(sequence_of_sequences[k]) == str:
                if sublist[j].isupper() == 1:
                    newlist = newlist + sublist[j]
    return newlist
# -----------------------------------------------------------------------------
